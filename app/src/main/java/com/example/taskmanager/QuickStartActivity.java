...
public class QuickStartActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		HuaweiIdAuthButton login = findViewById(R.id.HuaweiIdAuthButton) ;
		...
		if(!session.isLoggedIn()) {
			...
			View child = getLayoutInflater().inflate(R.layout.user_info, null);
			item.addView(child);
			HashMap<String,String> user = session.getUserDetails();

			Button signOut=findViewById(R.id.HuaweiIdSignOutButton);
			...
			Picasso.get().load(user.get(SessionManagement.KEY_AVATAR)).fit().centerCrop().into(pp);

			back.setOnClickListener(new View.OnClickListener(){
				@Override
				public void onClick(View v) {
					Intent i = new Intent(QuickStartActivity.this,MainActivity.class);
					startActivity(i);
				}
			});
	...
	private void dealWithResultOfSignIn(AuthAccount authAccount) {
		//获取帐号信息
		session.createLoginSession(authAccount.getDisplayName(),authAccount.getEmail(),authAccount.getAvatarUriString());
		finish();
